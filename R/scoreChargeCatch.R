...
#' @return This function returns a numeric vector with score for each peptide of resTab (even if \code{scale01=TRUE} minimum may be >0 if all peptides do contain charge-catching AAs)
...
#' 
#' @export
scoreChargeCatch <- function(resTab, pepCol="seq", scale01=TRUE, chargeMode="pos", silent=FALSE, debug=FALSE, callFrom=NULL) {
  ## Scoring of charge catching potential for peptides
  ## 
...
#' @param debug (logical) additional messages and objects exportet to current session for debugging
#' @param callFrom (character) allow easier tracking of messages produced
#' @return This function returns a matrix with charge-catching amino-acids and their assumed strength
#' @seealso \code{\link{fragmentSeq}}
#' @examples
#' .chargeCatchingAA()
#' @export
.chargeCatchingAA <- function(chargeMode="pos", silent=FALSE, debug=FALSE, callFrom=NULL){
